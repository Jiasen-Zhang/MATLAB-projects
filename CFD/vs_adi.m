...
%     %2nd order upwind
    Mi(2,ij)=0.5*(1+sign(u(2,ij))).*(uw(2,ij)-uw(1,ij))/(hi)...
        +0.5*(1-sign(u(2,ij))).*(uw(3,ij)-uw(2,ij))/(hi);
    Mi(ni-1,ij)=0.5*(1+sign(u(ni-1,ij))).*(uw(ni-1,ij)-uw(ni-2,ij))/(hi)...
        +0.5*(1-sign(u(ni-1,ij))).*(uw(ni,ij)-uw(ni-1,ij))/(hi);
    Mj(ii,2)=0.5*(1+sign(v(ii,2))).*(vw(ii,2)-vw(ii,1))/(hj)...
        +0.5*(1-sign(v(ii,2))).*(vw(ii,3)-vw(ii,2))/(hj);
    Mj(ii,nj-1)=0.5*(1+sign(v(ii,nj-1))).*(vw(ii,nj-1)-vw(ii,nj-2))/(hj)...
        +0.5*(1-sign(v(ii,nj-1))).*(vw(ii,nj)-vw(ii,nj-1))/(hj);
    %
    Mi(ii2,ij2)=0.5*(1+sign(u(ii2,ij2))).*...
        ((3*uw(ii2,ij2)-4*uw(ii2-1,ij2)+uw(ii2-2,ij2))/(2*hi)-...
        dt*(uw(ii2,ij2)-2*uw(ii2-1,ij2)+uw(ii2-2,ij2))/(2*hi^2))+...
        0.5*(1-sign(u(ii2,ij2))).*...
        ((-3*uw(ii2,ij2)+4*uw(ii2+1,ij2)-uw(ii2+2,ij2))/(2*hi)-...
        dt*(uw(ii2,ij2)-2*uw(ii2+1,ij2)+uw(ii2+2,ij2))/(2*hi^2));
    Mj(ii2,ij2)=0.5*(1+sign(v(ii2,ij2))).*...
        ((3*vw(ii2,ij2)-4*vw(ii2,ij2-1)+vw(ii2,ij2-2))/(2*hj)-...
        dt*(vw(ii2,ij2)-2*vw(ii2,ij2-1)+vw(ii2,ij2-2))/(2*hj^2))+...
        0.5*(1-sign(v(ii2,ij2))).*...
        ((-3*vw(ii2,ij2)+4*vw(ii2,ij2+1)-vw(ii2,ij2+2))/(2*hj)-...
        dt*(vw(ii2,ij2)-2*vw(ii2,ij2+1)+vw(ii2,ij2+2))/(2*hj^2));
