...
for i=1:N
    ...
    Ei=[r(xi);
        0;
        -r(zi)];
%     gradE=[6*r(xi),0,-6*r(zi);
%         0,0,0;
%         -6*r(zi),0,-6*r(xi)];
%     Ei=[3*r(xi)*r(xi)-3*r(zi)*r(zi);
%         0;
%         -6*r(xi)*r(zi)];
%     gradE=[12*r(xi)^2-12*r(zi)^2,0,-24*r(xi)*r(zi);
%         0,0,0;
%         -24*r(xi)*r(zi),0,12*r(zi)^2-12*r(xi)^2];
%     Ei=[4*(r(xi)^3)-12*r(xi)*r(zi)^2;
%         0;
%         4*(r(zi)^3)-12*r(zi)*r(xi)^2];
